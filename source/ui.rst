...
При первом запуске :program:`NextGIS QGIS` пользователь видит следующее окно.
...
Интерфейс :program:`NextGIS QGIS` разделяется на следующие области:
...
и панели инструментов организованы по-разному.
...
Все эти предустановки так же видны из `компоновщика карты <https://docs.nextgis.ru/docs_ngqgis/source/map_composer.html>`_. Вы можете составлять карты 
для печати на основе заданных вами комбинаций слоёв.
...
2. Выделить несколько слоёв, нажать правой кнопкой в окне слоёв и выбрать 
   ``Сгруппировать выделенное``. Выделенные слои автоматически поместятся в новую группу. 
