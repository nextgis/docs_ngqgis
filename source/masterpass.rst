...
Для безопасного хранения и использования чувствительной информации в QGIS — например, учётных данных, паролей и токенов доступа к различным сервисам — пользователь должен задать единый мастер-пароль.

Мастер-пароль запрашивается при первом сохранении зашифрованных данных и открывает доступ к защищённому хранилищу паролей QGIS. По умолчанию он запоминается только на время текущего рабочего сеанса, то есть до закрытия приложения.

Важно: мастер-пароль не заменяет пароли и настройки доступа к отдельным сервисам. Вам по-прежнему нужно отдельно настраивать подключение к каждому сервису внутри QGIS, например к вашей Веб ГИС NextGIS Web через NextGIS Connect.
