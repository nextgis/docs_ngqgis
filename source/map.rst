...
Для добавления слоя в формате на карту нажмите :menuselection:`Слой --> Добавить слой --> Добавить слой CSV`. Откроется окно :guilabel:`Создать слой из текстового файла`.
...
После открытия координат - подложите Mapnik, и проверьте, в правильное ли место земного шара попали координаты. Если они попали в другое место - значит вы перепутали широту и долготу. Нужно импортировать слой заново, и задать поля :guilabel:`X-координата` и :guilabel:`Y-координата` по-другому.
