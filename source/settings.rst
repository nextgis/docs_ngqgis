...
Подробнее о настройке системы координат можно прочитать `здесь <https://docs.nextgis.ru/docs_ngqgis/source/srs.html>`_.
...
.. note::
   ...
   При помощи этих настроек можно управлять видом параллельной кривой. Все эти настройки будут учитываться только при использовании GEOS 3.3 или более поздней версии.
...
* Параметры кэширования (путь к кэшу и его размер).
...
их в текстовое поле ниже, нажав кнопку "Добавить" (иконка со знаком плюса). 
...
В этом разделе можно настроить фильтры поиска, вызываемого панелью в левом нижнем углу экрана:
...
Подробнее о работе с проекциями и системами координат см. в `этом разделе <https://docs.nextgis.ru/docs_ngqgis/source/srs.html>`_.
...
На этой вкладке можно прописывать отношения между слоями проекта. Например, подтянуть дополнительные атрибуты из отдельной таблицы.

Добавьте таблицу в проект, она будет отображаться в списке слоёв как слой без геометрии.

Откройте меню Проект - Свойства проекта и на вкладке Отношения нажмите **Добавить отношение**. 

.. figure:: _static/reelations_add_ru.png
   :name: reelations_add_pic
   :align: center
   :width: 20cm

   Добавление отношения между слоем и таблицей

Укажите параметры отношения:

* Имя - так будет называться раздел с данными из прикреплённой таблицы в панели идентификации;
* Степень связи - ассоциация (обычная) и композиция (жёсткая зависимость);
* Отображение слоёв и полей: 

  * В колонке "родитель" укажите слой, к которому нужно добавить отношение, в колонке "дочерний" - таблицу, откуда нужно брать дополнительные данные. 
  * Выберите поле, по которому будут сопоставляться объекты. Например, по названию населённого пункта - в родительском слое это атрибут 'NAME', а в дополнительной таблице - колонка 'place'.

Нажмите **Ок** в диалоге добавления отношения, затем в диалоге настроек.

После этого при идентификации объекта будут отображаться помимо атрибутов из векторного слоя данные из таблицы.

.. figure:: _static/reelations_identify_ru.png
   :name: reelations_identify_pic
   :align: center
   :width: 18cm

   В панели идентификации отображаются данные из дополнительной таблицы
