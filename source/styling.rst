...
.. figure:: _static/ngq_styles_types.png
   :height: 6cm
   :align: center

   Примеры символов для точечных, линейных и полигональных слоёв.

* **Тип символьного слоя** - задаёт способ заливки: цветом, штриховкой, :term:`SVG`, маркерами, 
...
* Для точечных слоёв:
  ...
  * **SVG маркер**: отрисовка с использованием :term:`SVG` изображения.
...
* Для полигональных слоёв:
  ...
  * **Заливка SVG узором**: Заливка полигона :term:`SVG` изображением.
...
Рисунок иллюстрирует диалог отрисовки уникальными значениями из демонстрационного набора данных NextGIS QGIS:
...
3. Разместить файл в интернете и поставить его URL в качестве пути к значку SVG.
...
формате :term:`QML` в каталоге, где лежит слой, с тем же названием. Теперь, если вы будете 
добавлять этот слой как новый, то NextGIS QGIS подхватит этот стиль.

Пункт "Сохранить стиль" - позволяет сохранить его в другой файл, а так же в формат :term:`SLD`.
