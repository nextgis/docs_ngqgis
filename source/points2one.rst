...
* Group features by - указывает поле, которое будет использоваться для группировки объектов. Используется, когда нужно собрать несколько объектов а не один. Идентификатор указанный в поле должен быть уникальным для каждого объекта;
